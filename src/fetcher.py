"""Fetcher worker that pulls URLs from the frontier and fetches HTML.

Responsibilities of the fetcher worker:
* Respect semaphore concurrency limit.
* Respect robots.txt disallow rules.
* Respect rate_limit and robots.txt crawl-delay (whichever is larger).
* Handle HTTP errors:
        ...
        async with aiohttp.ClientSession() as session:
            while True:
                try:
                    ...
                    # No new items for 10 seconds so we assume the crawl is finished.
                    break

                await self.process(session, item)

    async def process(self, session: aiohttp.ClientSession, item: FrontierItem) -> None:
        url = item.url
        ...
        async with self._semaphore:
            ...
            # Respect rate_limit and robots.txt crawl-delay (whichever is larger).
            crawl_delay = self._robot_rules.crawl_delay()
            delay = max(self._config.rate_limit, crawl_delay or 0)
            if delay > 0:
            ...
            try:
                async with session.get(
                    ...
                    if resp.status == 404:
                        logger.warning("404 detected, dropping %s", url)
                ...
                await self.handle_failure(item, exc)
                return

            # Push to the parsing queue.
            await self._parse_queue.put(ParseItem(url=url, html=html))
            self._frontier.task_done()

    async def handle_failure(self, item: FrontierItem, exc: Exception) -> None:
